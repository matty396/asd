              ...
              <!-- /.card-header -->
                <div class="card-body"> 
                  ...
                  <form class="row g-3" action="{{url('cajaDetalle')}}" method="post" id="addCajaDetalle" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <input type="hidden" name="caja_id" id="caja_id" value="{{isset($caja)?$caja->id:''}}">
                    <input type="hidden" name="cliente_id" id="cliente_id" value="{{isset($cliente_id)?$cliente_id:''}}">
   ...
   $(document).ready(function(){
      ...
      $(".guardarYCopiar").click(function (e) {
       ...
       // TREMENDOOOOOOOO a ver que se puede hacer para que no cree el comprobante antes de entrar
        //fijate si en el serialize podes recibirlo como request y leer el caja id
        $.ajax({
                url: '{{url("cajaDetalle")}}',
                type: 'post',
                data: data  ,
