                    ...
                    <table class="table table-striped mt-4 detalles" id="tabla">
                        ...
                        <tbody id="tbody">
                            ...
                            <tr>
                                ...
                                <td>
                                <button onclick="eliminar('{{$value->id}}')" class="btn btn-danger btn-sm" >Borra</button>
                                </td>
                            </tr>
                            @endforeach        
                        </tbody>
   ...
   $(document).ready(function(){
      ...
      $(".borrar").click(function (e) {
        ...
        $.ajax({
                ...
                data: data  ,
                dataType: 'json',
                success: function(response){alert(JSON.stringify);
                    $('#venta_id').val(response.venta_id);
                    $('#comprobante').val(response.venta_id);
                    /*$('#codigo').val('');
                    $('#descripcion').val('');
    ...
    function eliminar(id){
        var data = $("#addventaDetalle").serialize()+ '&id=' + id;
        $.ajax({
                url: '{{url("ventaDetalle/borrarItem")}}',
                type: 'delete',
                data: data  ,
                dataType: 'json',
                success: function(response){
                    $('#venta_id').val(response.venta_id);
                    $('#comprobante').val(response.venta_id);alert("pasa");
                    $('#card-tabla').load(document.URL+  ' #card-tabla');
